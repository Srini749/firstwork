  ...
  const memoizedQuestions = useMemo(
    () =>
      formConfig.questions.map((question) => {
        ...
        return (
          <>
            <h3 className='left-align text-base font-semibold text-gray-800'>{question.title}</h3>
            {question.helperText && <p className='no-margin left-align text-sm text-gray-600'>{question.helperText}</p>}
            {renderQuestion(question)}
            {hasError && <div className='no-margin error-message'>{hasError}</div>}
          </>
        );
      }),
    ...
    <div className='max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-sm'>
      ...
      <form onSubmit={handleSubmit} className='space-y-6'>
        ...
        {!formConfig.isSubmitted && <button
          ...
          } preview-submit-button w-full bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2`}
